defmodule RadioBeamWeb.FilterControllerTest do
  ...
  describe "put/2" do
    test "successfully puts a filter", %{conn: conn, account: %{user_id: user_id}} do
      req_body = %{
  ...
  describe "get/1" do
    ...
    test "can retrive a user's previously put filter by ID", %{
      conn: conn,
    ...
    test "returns M_NOT_FOUND when no filter is known under the given ID", %{conn: conn, account: %{user_id: user_id}} do
