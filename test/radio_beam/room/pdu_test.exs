defmodule RadioBeam.Room.PDUTest do
  ...
  describe "new!/1" do
    test "creates a new %PDU{} with the given %AuthorizedEvent{} and prev_events" do
      ...
      assert %PDU{event: ^create_event, prev_event_ids: []} = PDU.new!(create_event, [], 0)

      message_event =
        Fixtures.authz_message_event(Fixtures.room_id(), Fixtures.user_id(), [create_event.id], "this is a test")

      assert %PDU{event: ^message_event, prev_event_ids: [^create_event_id], stream_number: 1} =
               PDU.new!(message_event, [create_event_id], 1)
    end
  end
end
