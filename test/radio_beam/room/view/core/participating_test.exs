defmodule RadioBeam.Room.View.Core.ParticipatingTest do
  ...
  describe "handle_pdu/3" do
    ...
    test "adds the room ID to `:all` when the PDU is a membership event", %{
      ...
      {:sent, room, _event_id, [pdu]} = Fixtures.send_room_membership(room, creator_id, user_id, :invite)
      view = Participating.new!()
      updated_view = Participating.handle_pdu(view, room.id, %{}, pdu)

      assert view.latest_known_join_pdus == updated_view.latest_known_join_pdus
      assert MapSet.new([room.id]) == updated_view.all

      {:sent, other_room, _event_id, [pdu]} =
        "11" |> Fixtures.room(creator_id) |> Fixtures.send_room_membership(creator_id, user_id, :invite)

      updated_view = Participating.handle_pdu(updated_view, other_room.id, %{}, pdu)
    ...
    test "adds the room ID to `:all` and `:latest_known_join_pdus` when the PDU is a join membership event", %{
      ...
      {:sent, room, _event_id, _pdus} = Fixtures.send_room_membership(room, creator_id, user_id, :invite)
      {:sent, room, _event_id, [join_pdu]} = Fixtures.send_room_membership(room, user_id, user_id, :join)
      view = Participating.new!()
      updated_view = Participating.handle_pdu(view, room.id, %{}, join_pdu)
