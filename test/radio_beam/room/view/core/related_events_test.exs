defmodule RadioBeam.Room.View.Core.RelatedEventsTest do
  ...
  describe "handle_pdu/3" do
    setup do
      ...
      {:sent, room, _event_id, _pdus} = Fixtures.send_room_membership(room, user_id, user_id, :join)

      %{room: room, creator_id: creator_id, user_id: user_id}
    end

    test "noop for unrelated events", %{room: room, creator_id: creator_id} do
      {:sent, _room, _event_id, [pdu]} = Fixtures.send_room_msg(room, creator_id, "hello")
      related_events = RelatedEvents.new!()
      assert ^related_events = RelatedEvents.handle_pdu(related_events, room.id, %{}, pdu)
    end
    ...
    test "tracks related events", %{room: room, creator_id: creator_id, user_id: user_id} do
      ...
      related_events =
        RelatedEvents.new!()
        |> RelatedEvents.handle_pdu(room.id, %{}, thread_root_pdu)
        |> RelatedEvents.handle_pdu(room.id, %{}, thread_pdu1)
        |> RelatedEvents.handle_pdu(room.id, %{}, thread_pdu2)
