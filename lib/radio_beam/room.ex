defmodule RadioBeam.Room do
  ...
  def upgrade(room_id, user_id, new_version, addl_creator_ids) do
    with {:ok, room} <- get(room_id),
         :ok <- ensure_can_send_tombstone(room, user_id),
         {:ok, new_room_id} <- create_upgraded_room(room, new_version, user_id, addl_creator_ids),
         :ok <- Database.rebind_aliases(room_id, new_room_id),
         {:ok, tombstone_id} <- put_tombstone_event(room_id, user_id, new_room_id, new_version) do
      Logger.info("upgraded #{room_id} -> #{new_room_id} (v#{new_version}). Tombstone event ID: #{tombstone_id}")
      ...
      # result.
      try_seal_old_room(room_id, user_id)
  ...
  defp ensure_can_send_tombstone(room, user_id) do
    if Room.Core.can_send_event?(room, user_id, "m.room.tombstone", "") do
      :ok
    else
      {:error, :unauthorized}
    end
  ...
  defp create_upgraded_room(old_room, new_version, creator_id, addl_creator_ids) do
    ...
    state_events_to_transfer =
      old_room.id
      |> Room.View.get_events!(creator_id, event_id_stream, false)
    ...
    create(creator_id, content: create_event_content, addl_state_events: state_events_to_transfer, version: new_version)
  end
  ...
  defp put_tombstone_event(room_id, user_id, new_room_id, new_version) do
    put_state(room_id, user_id, "m.room.tombstone", "", %{
      "body" =>
  ...
  defp try_seal_old_room(old_room_id, user_id) do
    with {:ok, event} <- get_state(old_room_id, user_id, "m.room.power_levels", "") do
      ...
      put_state(old_room_id, user_id, "m.room.power_levels", "", new_content)
    end
  end
