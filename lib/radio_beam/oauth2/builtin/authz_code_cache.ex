defmodule RadioBeam.OAuth2.Builtin.AuthzCodeCache do
  ...
  def put(code, code_challenge, client_id, %URI{} = redirect_uri, user_id, scopes) do
    GenServer.call(__MODULE__, {:put, code, code_challenge, client_id, redirect_uri, user_id, scopes})
  end
    ...
    case GenServer.call(__MODULE__, {:pop, code}) do
      {:ok, %{code_challenge: code_challenge, client_id: ^client_id, redirect_uri: ^redirect_uri} = bound_values} ->
        if code_challenge == Base.url_encode64(:crypto.hash(:sha256, code_verifier), padding: false) do
          {:ok, bound_values.user_id, bound_values.scopes}
        else
          {:error, :invalid_grant}
        end
  ...
  @impl GenServer
  def handle_call({:put, code, code_challenge, client_id, redirect_uri, user_id, scopes}, _from, state) do
