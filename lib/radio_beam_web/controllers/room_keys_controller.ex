defmodule RadioBeamWeb.RoomKeysController do
  ...
  def get_keys(conn, %{"version" => version} = params) do
    ...
    case KeyStore.fetch_room_keys_backup(user_id, version, room_id, session_id) do
  ...
  def put_keys(conn, %{"version" => version} = params) do
    ...
    case KeyStore.put_room_keys_backup(user_id, version, new_room_session_backups) do
      {:ok, backup_info} ->
  ...
  def delete_keys(conn, %{"version" => version} = params) do
    ...
    case KeyStore.delete_room_keys_backup(user_id, version, path_or_all) do
  ...
  def create_backup(conn, _params) do
    ...
    case KeyStore.create_room_keys_backup(user_id, algorithm, auth_data) do
      {:ok, version} ->
  ...
  def get_backup_info(conn, params) do
    ...
    with {:ok, version} <- version_result,
         {:ok, backup_info} <- KeyStore.fetch_backup_info(user_id, version) do
      json(conn, backup_info)
    else
  ...
  def put_backup_auth_data(conn, version) when is_integer(version) do
    ...
    case KeyStore.update_room_keys_backup_auth_data(user_id, version, algorithm, auth_data) do
      :ok ->
        json(conn, %{})
  ...
  def delete_backup(conn, %{"version" => version}) do
    case KeyStore.delete_room_keys_backup(conn.assigns.user_id, version) do
      :ok ->
        json(conn, %{})
